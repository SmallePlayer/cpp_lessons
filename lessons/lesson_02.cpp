// =============================================================================
// Урок 2: Переменные, типы данных и операторы
// =============================================================================
// Темы:
...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "   Урок 2: Переменные и типы данных" << std::endl;
    std::cout << "========================================\n" << std::endl;
    ...
    // В C++ есть НЕСКОЛЬКО целочисленных типов разного размера:
    //
    ...
    //
    // ВАЖНО: unsigned типы ОПАСНЫ! При вычитании могут "перетечь":
    //   unsigned int x = 0; x = x - 1; // → 4294967295 (НЕ -1!)
    ...
    // =====================================================================
    // 2. ТИПЫ С ФИКСИРОВАННЫМ РАЗМЕРОМ (C++11)
    // =====================================================================
    // Размеры short, int, long ЗАВИСЯТ от платформы!
    // Для гарантированного размера используйте типы из <cstdint>:
    ...
    // 4. char и bool
    // =====================================================================
    // char — хранит ОДИН символ (или число от -128 до 127).
    //   Символ заключается в ОДИНАРНЫЕ кавычки: 'A'
    //   Строка — в ДВОЙНЫЕ: "Hello"
    //
    ...
    std::cout << "a - b = " << (a_op - b_op) << std::endl;
    std::cout << "a * b = " << (a_op * b_op) << std::endl;
    std::cout << "a / b = " << (a_op / b_op) << " (ЦЕЛОЧИСЛЕННОЕ деление!)" << std::endl;
    ...
    //
    // 3. C-style cast (НЕ ИСПОЛЬЗУЕТСЯ!):
    ...
    // C++:    const int MAX_SIZE = 100;  (НЕЛЬЗЯ изменить — ошибка компиляции!)
    ...
    // GRAVITY = 10.0;  // ОШИБКА: assignment of read-only variable!
    // MAX_STUDENTS = 50;  // ОШИБКА: нельзя изменить constexpr!
    std::cout << std::endl;

    // =====================================================================
    // 10. auto — АВТОМАТИЧЕСКИЙ ВЫВОД ТИПА (C++11)
    // =====================================================================
    ...
    //
    // ВАЖНО: auto ТРЕБУЕТ инициализации!
    //   auto x;  // ОШИБКА — компилятор не знает тип!
