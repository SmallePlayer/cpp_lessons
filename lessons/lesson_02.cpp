// =============================================================================
//  2: еременные, типы данных и операторы
// =============================================================================
// Темы:
//   - бъявление и инициализация переменных
//   - елочисленные типы (int, short, long, long long)
//   - Типы с плавающей точкой (float, double)
//   - азмеры типов и sizeof
//   - рифметические операторы (+, -, *, /, %)
//   - елочисленное деление и остаток
//   - риведение типов (static_cast)
//   - онстанты (const, constexpr)
//   - итералы и суффиксы
//
// налогия с Python:
...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "   2: еременные и типы данных" << std::endl;
    std::cout << "========================================\n" << std::endl;

    // =====================================================================
    // 1. СЫ ТЫ
    // =====================================================================
    //  Python есть только int (неограниченной длины).
    //  C++ есть СЬ целочисленных типов разного размера:
    //
    //  Тип          | азмер (обычно) | иапазон
    //  -------------|-----------------|----------------------------------
    ...
    //
    // : unsigned типы СЫ! ри вычитании могут "перетечь":
    //   unsigned int x = 0; x = x - 1; // → 4294967295 ( -1!)
    //   екомендация:  используйте unsigned, кроме битовых операций.

    std::cout << "--- 1. елочисленные типы ---" << std::endl;
    ...
    // иапазоны:
    std::cout << "иапазон int: " << INT_MIN << " до " << INT_MAX << std::endl;
    std::cout << "иапазон long long: " << LLONG_MIN << " до " << LLONG_MAX << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "--- 2. иксированные типы ---" << std::endl;
    ...
    // 3. С С Щ Т
    // =====================================================================
    //  Python есть только float (это double в C++).
    //
    //  Тип         | азмер  | Точность           | Суффикс
    //  ------------|---------|--------------------|---------
    //  float       | 4 байта | ~7 значащих цифр   | f или F
    //  double      | 8 байт  | ~15 значащих цифр  | (по умолчанию)
    //  long double | 8-16    | ≥15 цифр           | l или L
    //
    // : исла с плавающей точкой  ТЫ!
    //   0.1 + 0.2 ≠ 0.3 (как и в Python!)
    //   то особенность IEEE 754, а не бага языка.

    std::cout << "--- 3. исла с плавающей точкой ---" << std::endl;
    ...
    // налог в Python: sys.getsizeof(x) (но показывает полный объём объекта)
    ...
    // =====================================================================
    // сё как в Python, :
    //   еление целых чисел: 7 / 2 = 3 ( 3.5!)
    //    Python 3: 7 / 2 = 3.5, 7 // 2 = 3
    //    C++: 7 / 2 = 3 (целочисленное деление!)
    //   ля дробного результата: 7.0 / 2 = 3.5
    //
    //   ператор % (остаток): 7 % 2 = 1
    //   ТТ ТЬ С Ы С (в C++)!
    //    Python % работает и с float.

    std::cout << "--- 6. рифметические операторы ---" << std::endl;
    ...
    // =====================================================================
    // ак в Python: +=, -=, *=, /=, %=
    //  C++ дополнительно есть: ++, -- (инкремент и декремент)
    //   ++x  — увеличить x на 1  использованием (prefix)
    //   x++  — увеличить x на 1 С использования (postfix)
    //
    ...
    // 8.  Т (TYPE CASTING)
    // =====================================================================
    //  Python: int(3.14) → 3, float(5) → 5.0, str(42) → "42"
    //
    //  C++ есть несколько видов приведения:
    //
    // 1. еявное (implicit) — компилятор делает сам:
    ...
    //
    // 3. C-style cast ( ТСЯ!):
    //    int x = (int)3.14;  // аботает, но опасно
    //
    // С используйте static_cast — он безопаснее и виднее в коде!

    std::cout << "--- 8. риведение типов ---" << std::endl;
    ...
    //   ак final в Java или UPPER_CASE в Python (по соглашению)
    //
    // constexpr (C++11) — значение известно на этапе Я
    //   омпилятор вычисляет значение заранее, до запуска программы
    //   то быстрее, чем const!
    //
    // Python: MAX_SIZE = 100  (по соглашению, но можно изменить!)
    // C++:    const int MAX_SIZE = 100;  (ЬЯ изменить — ошибка компиляции!)

    std::cout << "--- 9. онстанты ---" << std::endl;
    ...
    // 11.  (OVERFLOW)
    // =====================================================================
    //  Python целые числа не переполняются — они растут бесконечно.
    //  C++ — ЯТСЯ! то Undefined Behavior для signed типов!
    //
    // int max = INT_MAX;     // 2147483647
    // max = max + 1;         // UB! (обычно станет -2147483648)
    //
    // ля unsigned: «оборачивается» (wrap around)
    //   unsigned int x = 0;
    //   x = x - 1;  // → 4294967295 (UINT_MAX)

    std::cout << "--- 11. ереполнение ---" << std::endl;
    std::cout << "INT_MAX = " << INT_MAX << std::endl;
    std::cout << "INT_MIN = " << INT_MIN << std::endl;
    // емонстрация unsigned overflow (defined behavior):
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "  рок 2 завершён!" << std::endl;
    std::cout << "========================================\n" << std::endl;

    std::cout << "Я Я Т:" << std::endl;
    std::cout << "1. апишите программу-калькулятор для двух чисел" << std::endl;
    std::cout << "2. ычислите площадь круга (pi * r * r)" << std::endl;
    std::cout << "3. онвертируйте температуру: ельсий → аренгейт" << std::endl;
    std::cout << "4. оэкспериментируйте с sizeof для всех типов" << std::endl;
    std::cout << "5. опробуйте присвоить новое значение const переменной" << std::endl;
