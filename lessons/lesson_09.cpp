...
// 1. struct — СТЯ СТТ ЫХ
// =====================================================================
// руппирует несколько переменных в один тип.
...
struct Point {
    ...
    // етод в структуре (вполне допустимо):
...
struct Color {
    ...
    Color() : r(0), g(0), b(0) {}  // онструктор по умолчанию
...
public:
    // ─── СТТЫ ───
    // онструктор — специальная функция, вызывается при создании объекта.
    // мя = имя класса. ет возвращаемого типа.
    //
    // Python: def __init__(self, ...):
    // C++:    ClassName(параметры) : инициализация { тело }
    //
    // СС   (member initializer list):
    //   BankAccount(std::string o) : owner(o), balance(0.0) { }
    //   то Ш, чем присваивание в теле конструктора!
    //   оля инициализируются  выполнения тела конструктора.

    // онструктор по умолчанию:
    BankAccount() : owner("Unknown"), balance(0.0), account_id(next_id++) {
        std::cout << "  [онструктор] Создан аккаунт #" << account_id << std::endl;
    }

    // араметризированный конструктор:
    BankAccount(const std::string& o, double b)
        : owner(o), balance(b), account_id(next_id++) {
        std::cout << "  [онструктор] Создан аккаунт #" << account_id
                  << " для " << owner << std::endl;
    }

    // ─── СТТ ───
    // ызывается при Т объекта (выход из scope, delete).
    // спользуется для освобождения ресурсов.
    //
    // Python: def __del__(self):
    // C++:    ~ClassName() { }
    ~BankAccount() {
        std::cout << "  [еструктор] ккаунт #" << account_id
    ...
    bool withdraw(double amount) {
        ...
        std::cout << "  едостаточно средств!" << std::endl;
        return false;
    }

    void print_info() const {
        std::cout << "  ккаунт #" << account_id << " | "
                  << owner << " | аланс: " << balance << std::endl;
    }
...
// нициализация статического члена (ЯТЬ вне класса!):
...
public:
    ...
    // ерегрузка оператора << для вывода (friend функция)
...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "   9: Структуры и лассы" << std::endl;
    std::cout << "========================================\n" << std::endl;
    ...
    std::cout << "асстояние: " << p1.distance_to(p2) << std::endl;
    ...
    Color red{255, 0, 0};
    Color white;  // онструктор по умолчанию
    std::cout << "red = "; red.print(); std::cout << std::endl;
    std::cout << "white = "; white.print(); std::cout << std::endl;
    std::cout << std::endl;

    // --- 2. class ---
    std::cout << "--- 2. лассы (class) ---" << std::endl;
    {
        BankAccount acc1;
        BankAccount acc2{"лексей", 1000.0};

        acc2.print_info();
        acc2.deposit(500.0);
        acc2.withdraw(200.0);
        acc2.withdraw(2000.0);  // едостаточно средств
        acc2.print_info();

        std::cout << "сего аккаунтов создано: " << BankAccount::get_total_accounts() << std::endl;
        std::cout << std::endl;
    }  // acc1 и acc2 уничтожаются — вызываются деструкторы!
    std::cout << std::endl;

    // --- 3. ерегрузка операторов ---
    std::cout << "--- 3. ерегрузка операторов ---" << std::endl;
    ...
    // --- 4. ассив объектов ---
    std::cout << "--- 4. ектор объектов ---" << std::endl;
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "  рок 9 завершён!" << std::endl;
    std::cout << "========================================\n" << std::endl;
    ...
    std::cout << "3. ерегрузите оператор [] для класса Matrix" << std::endl;
    std::cout << "4. еализуйте класс String с конструктором копирования" << std::endl;
